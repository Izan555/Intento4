...
function arenaSweep() {
  ...
  if (lines > 0) {
    const points = lines * 10 * rowCount;
    player.score += points;
    player.lines += lines;

    // Cambio de nivel por puntaje
    const threshold = (player.level < 15) ? 50 : 100;
    if (player.score >= (player.level + 1) * threshold) {
      player.level++;
...
function togglePause() {
  ...
  if (isPaused) {
    ...
    lastTime = performance.now(); // reset tiempo para evitar salto
    update();
  }
}

// Reanudar desde reset
function resumeGame() {
  isPaused = false;
...
playerReset();
updateScore();
update();// Controles por botones táctiles
...
document.getElementById('btn-rotate').addEventListener('click', () => {
  playerRotate(-1); // o cambia a 1 si prefieres rotación horaria
});
